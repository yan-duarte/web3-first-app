    ...
    return (
        <nav className='w-full flex md:justify-center justify-between items-center p-4'>
            ...
            <ul className='text-white md:flex hidden list-none flex-row justify-between items-center flex-initial'>
                {["Market", "Exchange", "Tutorials", "Wallets"].map((item, index) => (<NavbarItem title={item} key={item+index}/>))}
                ...
                {toggleMenu && (
                    <ul className='z-10 fixed top-0 -right-2 p-3 w-[70vw] h-screen shadow-2x1 md:hidden list-none flex flex-col justify-start items-end rounded-md blue-glassmorphism text-white animate-slide-in'>
                        ...
                        <li className='text-xl w-full my-2'>
                            ...
                            {["Market", "Exchange", "Tutorials", "Wallets"].map((item, index) => (<NavbarItem title={item} key={item+index} classProps='my-2 text-lg'/>))}
                        </li>
                    </ul>
                )}
            </div>
